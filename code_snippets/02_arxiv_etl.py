...
def convert_document(input_path: Path, output_dir: Path, ocr_backend: str):
    ...
    """
    _log.info(f"Starting conversion with OCR backend: {ocr_backend}")
    ...
    _log.info(f"Document converted in {elapsed_time:.2f} seconds using {ocr_backend}.")
...
def convert_with_backend(input_doc_paths: list[Path], ocr_backend: str, output_dir: Path | str):
    ...
    if failure_count > 0:
        _log.error(f"Conversion failed with {failure_count} failures and {partial_success_count} partial successes.")
    
    _log.info(f"Files to process: {files_to_process}")
        ...
        for url in urls:
            ...
            if metadata_response.status_code == 200:
                ...
                if entry is not None:
                    ...
                    _log.info(f"Found paper title: {title}")
                else:
                    _log.warning(f"No entry found for {url}")
                    title = f"Untitled_{arxiv_id}"  # Fallback title
            else:
                _log.warning(f"Failed to fetch metadata for {url}: {metadata_response.status_code}")
            ...
            if pdf_response.status_code == 200:
                ...
                _log.info(f"Downloaded PDF from {url} successfully!")
            else:
                _log.warning(f"Failed to download PDF from {url}.")
...
def check_tesseract_installed():
    ...
    try:
        ...
        _log.info("Tesseract is installed and available.")
        return True
    except subprocess.CalledProcessError:
        _log.warning("Tesseract command is not available.")
        return False
...
def load_arxiv_yaml(yaml_path: Path) -> dict[str, BytesIO]:
    try:
        ...
        if not urls:
            _log.error("No URLs found in arxiv.yaml")
            return

        _log.info(f"Found {len(urls)} URLs in arxiv.yaml")

        
    except FileNotFoundError:
        _log.error(f"Could not find arxiv.yaml at {yaml_path}")
        return
    except yaml.YAMLError as e:
        _log.error(f"Error parsing arxiv.yaml: {e}")
        return
...
def main():
    ...
    input_doc_paths = [DocumentStream(name=title, stream=stream) 
                       for title, stream in pdf_streams.items()]
